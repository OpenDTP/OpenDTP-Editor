...
function Renderer(t,i){this.node=t,this.map=i,this.zoom=1,this.load=function(){this.node=t,this.map=i,this.zoom=1},this.render=function(t,i){var e;void 0===t&&(t=0),!0===i&&$(this.node).html("");var h,o,a,s,d;for(var n in this.map.spreads[t].pages){e=this.createSVG("svg",{height:"100%",width:"100%"}),d=this.map.spreads[t].pages[n];for(var r in d.blocs)h=d.blocs[r].x/this.map.width*100,o=d.blocs[r].y/this.map.height*100,a=d.blocs[r].width/this.map.width*100,s=d.blocs[r].height/this.map.height*100,e.append(this.createSVG("rect",{x:h.toFixed(2)+"%",y:o.toFixed(2)+"%",width:a.toFixed(2)+"%",height:s.toFixed(2)+"%","class":"bloc bloc-"+d.blocs[r].type,"data-name":r,"data-spread":t,"data-page":n}));e.css("background-image",'url("'+d.preview+'")'),$(this.node).append(e)}this.resizeMap(),this.node.trigger("change")},this.resizeMap=function(t){var i=void 0===t?this:t.data,e=$(i.node).find("svg"),h=$(e[0]).outerWidth()-$(e[0]).width(),o=$(e[0]).outerHeight()-$(e[0]).height(),a=$(i.node).width(),s=$(i.node).height(),d=i.map.width*e.length/i.map.height,n=a/s;d>n?($(e).attr("width",a/e.length*this.zoom-h+"px"),$(e).attr("height",(a/d*this.zoom).toFixed(0)-o+"px")):($(e).attr("height",s*this.zoom-o+"px"),$(e).attr("width",(s*d/e.length*this.zoom).toFixed(0)-h+"px"))},this.filter=function(t){var i=$(this.node).find("svg");"all"!==t&&i.find(".bloc").hide(),"none"!==t&&"all"!==t&&i.find(".bloc-"+t).show(),"all"===t&&i.find(".bloc").show()},this.createSVG=function(t,i){var e=$(document.createElementNS("http://www.w3.org/2000/svg",t));for(var h in i)e.attr(h,i[h]);return e}}
...
MAPEDITOR.plugins.push(function(){var a,t,e="flatplan",n=33.3333;this.load=function(e){var n=$('<div class="header"><di class="title">Flatplan<div></div>'),i=$('<div class="hide-toggle">&gt;</div>');a=$('<div class="flatplan"></div>'),t=$('<div class="flatplan-content"></div>'),n.prepend(i),this.flatplan_content.append(n),this.flatplan_content.append(this.flatplan),this.loadFlatplan(e),e.right_pannel.append(this.flatplan_content),n.click({flatplan:this.flatplan,hide:i},function(a){a.data.flatplan.toggle(),$(a.data.flatplan).is(":visible")?$(a.data.hide).removeClass("show"):$(a.data.hide).addClass("show")})},this.loadFlatplan=function(a){for(var t,e,i,d,s=a.map.spreads.length,p=0;s>p;p++){d=a.map.spreads[p].pages.length,e=$('<div class="spread" data-index="'+p+'"></div>'),i=1/a.map.spreads[p].nb_pages*100,i=i>this.max_width?n:i;for(var l in a.map.spreads[p].pages)t=$('<div class="page"><img class="page" src="'+a.map.spreads[p].pages[l].preview+'" width="100%" /></div>'),t.css("width",i.toFixed(2)+"%"),e.append(t);e.click(a,function(a){var t=$(a.target);a.data.properties.current_spread=void 0===t.attr("data-index")?parseInt(t.parents(".spread").attr("data-index")):parseInt(t.attr("data-index")),a.data.renderer.render(a.data.properties.current_spread,!0)}),this.flatplan.append(e)}},this.__defineGetter__("name",function(){return e}),this.__defineGetter__("max_width",function(){return n}),this.__defineGetter__("flatplan",function(){return a}),this.__defineGetter__("flatplan_content",function(){return t}),this.__defineSetter__("name",function(a){e=a}),this.__defineSetter__("max_width",function(a){n=a})});
